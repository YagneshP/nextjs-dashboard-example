...
export default function Search({ placeholder }: { placeholder: string }) {
  ...
  const handleSearch = useDebouncedCallback((term: string) => {
    ...
    if (term) {
      ...
      params.delete("query"); // if search term is empty delete the prop
    }
    replace(`${pathname}?${params.toString()}`); //replace update the url with current pathname and param stringify
  }, 300);
  return (
